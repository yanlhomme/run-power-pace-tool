...
SOFT_SURFACE_FACTOR = (
    1.055  # Soft surface factor for grass (trail) wrt asphalt (experiment based value)
)
...
def estimate_frontal_area(height_m, weight_kg):
    ...
    return max(0.1, 0.18964 * height_m + 0.00215 * weight_kg - 0.07861)
...
def gpx_to_df(gpx_file, custom_surfaces=None):
    ...
    if custom_surfaces is None and False:
        ...
        try:
            ...
            st.warning(
                f"Could not fetch surface data: {str(e)}. Using default paved surface."
            )
        ...
        if len(surfaces) < len(df):
            surfaces = surfaces + [0] * (
                len(df) - len(surfaces)
            )  # pad with paved surfaces if needed
...
def datetime_to_timedelta(dt):
    ...
    return datetime.combine(datetime.today(), dt) - datetime.combine(
...
def compute_pace_smoothing_power_coeff(grade, coeff_max_up, coeff_max_down, alpha):
    coeff_up = (
        np.arctan(grade * alpha) / np.arctan(0.45 * alpha) * coeff_max_up + 1
    ) * (grade >= 0)
    coeff_down = (
        np.arctan(grade * alpha) / np.arctan(0.45 * alpha) * coeff_max_down + 1
    ) * (grade < 0)
...
def main_computation(
    ...
    df = pd.concat(
        [
            tmp1.iloc[:-1].reset_index(),
            tmp2.iloc[1:].reset_index().drop(columns=["surface"]),
        ],
        axis=1,
    )
    ...
    if resample_gpx_points is not None and resample_gpx_points > 1:
        df = (
            df.groupby(np.arange(len(df)) // resample_gpx_points)
            .agg(
                {
                    "lat0": "first",
                    "lon0": "first",
    ...
    def Ci(row):
        ...
        return (
            ...
            * eff
            * (SOFT_SURFACE_FACTOR if row["surface"] == 1 else 1.0)
        )
    ...
    def total_time_and_derivative(
        p, return_sum: bool = True
    ) -> tuple[np.ndarray, np.ndarray]:
        p_corrected = p * compute_pace_smoothing_power_coeff(
            df["grade"],
            pace_smoothing_power_coeff_max_up,
            pace_smoothing_power_coeff_max_down,
            pace_smoothing_power_alpha,
        )

        deltas = np.sqrt(df["bi"] ** 2 * p_corrected**2 + df["di"])

        times_p = 1 / (
            np.cbrt(df["bi"] * p_corrected + deltas)
            + np.cbrt(df["bi"] * p_corrected - deltas)
        )  # Solves the power equation for time
        ...
        derivative_p = (
            ...
            * df["bi"]
            * (
                (1 + df["bi"] * p_corrected / deltas)
                * (1 / np.cbrt((df["bi"] * p_corrected + deltas) ** 2))
                + (1 - df["bi"] * p_corrected / deltas)
                * (1 / np.cbrt((df["bi"] * p_corrected - deltas) ** 2))
            )
            / (
    ...
    def P(v, ci_val):
        return m * ci_val * v + 0.5 * rho * CdA * v**3

    mean_speed = df["horiz_dist"].sum() / target_total_time  # m/s
    p_init = P(
        mean_speed, Ci({"grade": 0.0, "surface": 1})
    )  # initial guess based on mean speed
    ...
    compute_segments(
        df, segment_length
    )  # groups GPS points into segments of segment_length (for plotting)
...
def get_summary_table(
    ...
    avg_power = (df["power"] * df["time"]).sum() / df[
        "time"
    ].sum()  # average power in W
    avg_equiv_cycling_power = (
        avg_power * RUNNING_METABOLIC_POWER_TO_CYLCING_MECHANICAL_POWER
    )  # convert to cycling equivalent power
    ...
    paved_total_distance = (
        df.loc[df["surface"] == 0, "horiz_dist"].sum() / 1000
    )  # convert to km
    paved_prct = (
        f"{paved_total_distance / total_distance:.1%}" if total_distance > 0 else "N/A"
    )
    unpaved_prct = (
        f"{1 - paved_total_distance / total_distance:.1%}"
        if total_distance > 0
        else "N/A"
    )
...
def compute_pace_per_km_plan(df: pd.DataFrame):
    pace_plan = {
        "start": [],
        "end": [],
        "distance_m": [],
    ...
    total_distance_m = df["horiz_dist"].sum()
    total_distance = total_distance_m / 1000  # convert to km
    n_km = int(np.ceil(total_distance))
    for km in range(n_km):
        max_dist = min((km + 1) * 1000, total_distance_m)
        df_km = df.loc[
            (df["cum_horiz_d"] >= km * 1000)
            & (df["cum_horiz_d"] <= max_dist)
            & (df["horiz_dist"] > 0),
            ["horiz_dist", "time", "cum_horiz_d"],
        ].copy()
        index_next = (
            df_km.index[-1] + 1 if (df_km.index[-1] + 1) < len(df) else df_km.index[-1]
        )
        if len(df_km) == 0:
            pace_plan["start"].append(np.nan)
            pace_plan["end"].append(np.nan)
            pace_plan["distance_m"].append(0)
            pace_plan["time_s"].append(0)
            pace_plan["pace"].append(np.nan)
            pace_plan["pace_str"].append("N/A")
            continue
        # Make the df_km holds exactly 1 km by adding partial segments at start and end if needed
        if df_km["cum_horiz_d"].iloc[0] - df_km["horiz_dist"].iloc[0] < km * 1000:
            # Add partial segment at start
            index = df_km.index[0]
            horiz_d = df_km.loc[index, "horiz_dist"]
            new_horiz_dist = df_km.loc[index, "cum_horiz_d"] - km * 1000
            df_km.loc[index, "horiz_dist"] = new_horiz_dist
            df_km.loc[index, "time"] = df_km.loc[index, "time"] * (
                new_horiz_dist / horiz_d
            )
            df_km.loc[index, "cum_horiz_d"] = km * 1000
        if df_km["cum_horiz_d"].iloc[-1] < max_dist:
            # Add partial segment at end
            row = df.iloc[index_next][["horiz_dist", "time", "cum_horiz_d"]].copy()
            horiz_dist_needed = max_dist - df_km["cum_horiz_d"].iloc[-1]
            row["horiz_dist"] = horiz_dist_needed
            row["time"] = row["time"] * (
                horiz_dist_needed / df.iloc[index_next]["horiz_dist"]
            )
            row["cum_horiz_d"] = max_dist
            df_km = pd.concat([df_km, pd.DataFrame([row])], ignore_index=True)
        df_km["cum_horiz_d"] = km * 1000 + df_km["horiz_dist"].cumsum()
        time_km = df_km["time"].sum()
        pace_km = time_km / 60 / (df_km["horiz_dist"].sum() / 1000)  # min/km
        pace_plan["start"].append(
            df_km["cum_horiz_d"].iloc[0] - df_km["horiz_dist"].iloc[0]
        )
        pace_plan["end"].append(df_km["cum_horiz_d"].iloc[-1])
        pace_plan["distance_m"].append(df_km["horiz_dist"].sum())
        pace_plan["time_s"].append(time_km)
        pace_plan["pace"].append(pace_km)
        pace_plan["pace_str"].append(fmt_pace(pace_km))
    return pd.DataFrame(pace_plan)
...
gpx_file = st.sidebar.file_uploader(
    "Upload GPX file", type=["gpx"], on_change=reset_custom_surfaces
)
# Add a selection to select whether to display the Pace computation or the surface classification page
selected_page = st.sidebar.selectbox(
    "Select page",
    ["Pace Computation", "Surface Classification"],
    index=0,
    key="page_selector",
    help="By selecting 'Surface Classification', you can manually classify the surfaces of the GPX track. The 'Pace Computation' page will compute the pace and power based on the GPX track and the provided parameters.",
)
segment_length = st.sidebar.number_input(
    "Plots Segment length (m)",
    value=100,
    on_change=on_input_change,
    help="Length of segments for the plots. The GPX file will be segmented into groups of this length for the map and elevation profile.",
)
st.sidebar.markdown("---")
...
height = st.sidebar.number_input(
    "Height (m)", value=1.75, min_value=1.2, max_value=2.2, on_change=on_input_change
)
eff = st.sidebar.slider(
    ...
    1.0,
    0.01,
    help="Efficiency multiplier for the power model. 1.0 corresponds to a group of 10 elite athletes practicing endurance moutain running (men age 32.6 ± 7.5 yr, body mass 61.2 ± 5.7 kg, maximal O2 consumption (VO2 max) 68.9 ± 3.8 ml/min/kg). Lower values correspond to more efficient runners, higher values to less efficient runners.",
)
...
pace_smoothing_power_coeff_max_up = (
    pace_smoothing_power_coeff_max_up_raw / 100.0
)  # convert to fraction
...
pace_smoothing_power_coeff_max_down = (
    pace_smoothing_power_coeff_max_down_raw / 100.0
)  # convert to fraction
...
y = compute_pace_smoothing_power_coeff(
    x,
    pace_smoothing_power_coeff_max_up,
    pace_smoothing_power_coeff_max_down,
    pace_smoothing_power_alpha,
)
fig_smoothing_coeff.add_trace(
    go.Scatter(x=x, y=y, mode="lines", name="Smoothing Coeff.")
)
...
target_mode = st.sidebar.select_slider(
    "Target", ["Total time", "Pace"], label_visibility="hidden"
)
if target_mode == "Total time":
    target_total_time_dt = st.sidebar.time_input(
        "Target time",
        value=datetime.now().replace(hour=1, minute=0, second=0, microsecond=0),
        step=timedelta(minutes=1),
        on_change=on_input_change,
    )
    target_total_time = datetime_to_timedelta(
        target_total_time_dt
...
if gpx_file and (should_run or selected_page == "Surface Classification"):
    ...
    st.session_state.custom_surfaces = df[
        "surface"
    ].tolist()  # store the surfaces for later use
    ...
    if selected_page == "Surface Classification":
        ...
        st.write(
            "To change the surface from one GPS point to another, click on the starting point of the segment you want to change (it'll be marked in red), then click on the end point of the segment. The surface will toggle between paved and unpaved.\nTo remove the selected starting point, click on it again."
        )
        ...
        """.replace(
                "<paved_color>", surface_colors[0]
            ).replace(
                "<unpaved_color>", surface_colors[1]
            ),
        ...
        if not st.session_state.is_surface_rerun:
            if map_data.get("last_clicked"):
                ...
                closest_point = (
                    df["lat0"].iloc[closest_idx],
                    df["lon0"].iloc[closest_idx],
                )
                geod = pyproj.Geod(ellps="WGS84")
                _, _, horiz_d = geod.inv(
                    closest_point[1], closest_point[0], coord[1], coord[0]
                )
                if (
                    horiz_d < 20
                ):  # if the clicked point is within 20m of the closest point
                    if closest_point not in st.session_state.endpoints:
                        ...
                        if len(st.session_state.endpoints) >= 2:
                            ...
                            start_idx = df[
                                (df["lat0"] == endpoints[0][0])
                                & (df["lon0"] == endpoints[0][1])
                            ].index[0]
                            end_idx = df[
                                (df["lat0"] == endpoints[1][0])
                                & (df["lon0"] == endpoints[1][1])
                            ].index[0]
                            if end_idx < start_idx:
                                start_idx, end_idx = end_idx, start_idx
                            current_suface = (
                                df["surface"].iloc[start_idx : end_idx + 1].mode().min()
                            )
                            new_surface = (
                                1 - current_suface
                            )  # toggle between paved (0) and unpaved (1)
                            st.session_state.custom_surfaces[
                                start_idx : end_idx + 1
                            ] = [new_surface] * (end_idx - start_idx + 1)
                            st.session_state.endpoints = []  # reset endpoints
                    else:
                        st.session_state.endpoints = []  # reset endpoints
        ...
        if (
            st.session_state.custom_surfaces is not None
            and sum(st.session_state.custom_surfaces) > 0
        ):
            st.button(
                "Reset to all paved",
                on_click=button_reset_custom_surfaces,
                help="Reset all surfaces to paved. This will remove all custom surface classifications and revert to the default paved surface.",
            )
    elif should_run:
        ...
        tooltips_location = zip(
            df[["lat0", "lat1"]].mean(axis=1), df[["lon0", "lon1"]].mean(axis=1)
        )
        tootips_text = df.apply(
            lambda row: f"{row['cum_horiz_d_km']:.2f} km<br>{row['smooth_pace_str']}",
            axis=1,
        ).tolist()
        ...
        for _, group in df.groupby("idx_segment"):
            cum_dist0 = (
                group["cum_horiz_d_km"] - group["horiz_dist"] / 1000
            )  # convert to km
        ...
        def agg_summary(dfgr):
            ...
            return pd.Series(
                {"horiz_dist": horiz_dist, "pace": avg_pace, "power": avg_power}
            )

        df["grade_cat"] = pd.cut(df["grade"], bins=bins, labels=labels)
        summary = (
            df.groupby("grade_cat", observed=False)
            .apply(agg_summary, include_groups=False)
            .reset_index()
        )
        ...
        for _, s in summary.iterrows():
            if s["horiz_dist"] < 0.1:
                continue
            st.write(
                f"{nice_grade_cat(s['grade_cat'])}: {s['horiz_dist']:.2f} km @ {fmt_pace(s['pace'])}/km -> {s['power']:.1f} W"
            )
        ...
        pace_plan_df = compute_pace_per_km_plan(df)
        pace_plan_df["start_km"] = (pace_plan_df["start"] / 1000).apply(
            lambda x: f"{x:.1f} km"
        )
        pace_plan_df["end_km"] = (pace_plan_df["end"] / 1000).apply(
            lambda x: f"{x:.1f} km"
        )
        pace_plan_df["cum_time"] = pace_plan_df["time_s"].cumsum()
        pace_plan_df["cum_time_str"] = pace_plan_df["cum_time"].apply(fmt_time)
        pace_plan_df = pace_plan_df[
            ["start_km", "end_km", "pace_str", "cum_time_str"]
        ].rename(
            columns={
                "start_km": "Start",
                "end_km": "End",
                "pace_str": "Pace",
                "cum_time_str": "Cumulative Time",
            }
        )

        st.subheader("📋 Paces Detail per km")
        st.dataframe(
            pace_plan_df,
            hide_index=True,
            use_container_width=False,
